...
class AgentAggSgtGp(AgentBase):#AgentMbLnrFwdGatFwdLnrBwdGatBwd
    def __init__(self):
        self.path = None
...
#            print(self.hyperConfigIndex)
        ...
        with open(iptConfigFileName) as json_file:
            ...
            self.timeLimitAggSgt = config["timeLimitAggSgt"]
    ...
    def compute_occuMeas_update(self, trainEpisodeIndex, policyUpdateIndex):#, trainEpisodeIndex, nnTrainStepIndex):
        ...
        if self.algoType == "AggSgtGp":
            self.solve_aggSgtGp_opt_update(trainEpisodeIndex, policyUpdateIndex)

    def solve_aggSgtGp_opt_eval(self, trainEpisodeIndex, policyEvalIndex):#, estEqOptConstr):#estGroupProb, estTransProbFirstGroup, estTransProbSecondGroup, estRewardFirstGroup, estRewardSecondGroup, estEqOptConstr):
        firstGroupIndex = 0
        secondGroupIndex = 1
        ...
        if True:
            ...
            agg.setParam(GRB.param.TimeLimit, self.timeLimitAggSgt)
    ...
    def solve_aggSgtGp_opt_update(self, trainEpisodeIndex, policyUpdateIndex):#, estEqOptConstr):#estGroupProb, estTransProbFirstGroup, estTransProbSecondGroup, estRewardFirstGroup, estRewardSecondGroup, estEqOptConstr):
        firstGroupIndex = 0
        secondGroupIndex = 1
            ...
            for scoreIndex in range(self.numScores):
                for isQualIndex in range(2):
                    for timeStep in range(self.numNnTrainSteps-1, 0, -1):
                        expr = 0
                        for decSgt1Index in range(2):
                            expr += self.groupList[groupIndex].occuMeasNpForUpdateEval[scoreIndex][isQualIndex][decSgt1Index][timeStep]
                        for scoreSgt1Index in range(self.numScores):
                            for isQualSgt1Index in range(2):
                                for decSgt1Index in range(2):
                                    expr -= self.groupList[groupIndex].occuMeasNpForUpdateEval[scoreSgt1Index][isQualSgt1Index][decSgt1Index][timeStep-1] * self.groupList[groupIndex].estTransProb[scoreIndex][isQualIndex][scoreSgt1Index][isQualSgt1Index][decSgt1Index]
...
#                print("groupIndex, timeStep: ", expr)
