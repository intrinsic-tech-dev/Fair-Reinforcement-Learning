...
class AgentEqOptCvSgtGp(AgentBase):#AgentMbLnrFwdGatFwdLnrBwdGatBwd
    def __init__(self):
        self.path = None
...
#            print(self.hyperConfigIndex)
        ...
        with open(iptConfigFileName) as json_file:
            ...
            self.timeLimitEqOptCvSgt = config["timeLimitEqOptCvSgt"]
    ...
    def compute_occuMeas_update(self, trainEpisodeIndex, policyUpdateIndex):#, trainEpisodeIndex, nnTrainStepIndex):
        ...
        if self.algoType == "EqOptCvSgtGp":#eqOptSgt
            self.solve_eqOptCvSgtGp_opt_update(trainEpisodeIndex, policyUpdateIndex)#estGroupProb, estEqOptConstr)#, estTransProbFirstGroup, estTransProbSecondGroup, estRewardFirstGroup, estRewardSecondGroup, estEqOptConstr)

    def solve_eqOptCvSgtGp_opt_update(self, trainEpisodeIndex, policyUpdateIndex):#, estEqOptConstr):#estGroupProb, estTransProbFirstGroup, estTransProbSecondGroup, estRewardFirstGroup, estRewardSecondGroup, estEqOptConstr):
        firstGroupIndex = 0
        secondGroupIndex = 1
        if True:
            ...
            objSum = 0.5 * objSum - 0.125 * objSum * objSum
            #fairness constr
            for timeStep in range(self.numNnTrainSteps):#-1, -1, -1):
                exprLhs = 0
            ...
            qcqp.setParam(GRB.param.TimeLimit, self.timeLimitEqOptCvSgt)
