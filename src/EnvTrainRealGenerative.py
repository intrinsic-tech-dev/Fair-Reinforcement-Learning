...
class GroupEnv(object):
    def __init__(self):
        ...
        self.transProbScoreCondScoreLag1IsQualVal1Dec1ListList = None
        self.transProbScoreCondScoreLag1IsQualVal1Dec0ListList = None
        self.transProbScoreCondScoreLag1IsQualVal0Dec1ListList = None
        self.transProbScoreCondScoreLag1IsQualVal0Dec0ListList = None
    ...
    def init(self, numScores, minScore, numNnTrainSteps, scoreScoreLag1IsQualVal1Dec1DnWgtDisList, scoreScoreLag1IsQualVal1Dec1EqWgtDis, scoreScoreLag1IsQualVal1Dec1UpWgtDisList, scoreScoreLag1IsQualVal1Dec0DnWgtDisList, scoreScoreLag1IsQualVal1Dec0EqWgtDis, scoreScoreLag1IsQualVal1Dec0UpWgtDisList, scoreScoreLag1IsQualVal0Dec1DnWgtDisList, scoreScoreLag1IsQualVal0Dec1EqWgtDis, scoreScoreLag1IsQualVal0Dec1UpWgtDisList, scoreScoreLag1IsQualVal0Dec0DnWgtDisList, scoreScoreLag1IsQualVal0Dec0EqWgtDis, scoreScoreLag1IsQualVal0Dec0UpWgtDisList, pmfScoreList, probIsQualCondScoreList, rewardCoefIsQual1Dec1, rewardCoefIsQual0Dec1, rewardCoefDec0, initProbTrue, transProbTrue, rewardTrue):

        self.minScore = minScore
        self.numScores = numScores
        self.numNnTrainSteps = numNnTrainSteps
 
        self.transProbScoreCondScoreLag1IsQualVal1Dec1ListList = []
        for i in range(self.numScores):
            probWgtList = scoreScoreLag1IsQualVal1Dec1DnWgtDisList[1:(i+1)][::-1] + [scoreScoreLag1IsQualVal1Dec1EqWgtDis] + scoreScoreLag1IsQualVal1Dec1UpWgtDisList[1:(self.numScores-i)]
            nrmProbWgtList = [probWgt * 1.0 / sum(probWgtList) for probWgt in probWgtList]
            self.transProbScoreCondScoreLag1IsQualVal1Dec1ListList.append(nrmProbWgtList)

        self.transProbScoreCondScoreLag1IsQualVal1Dec0ListList = []
        for i in range(self.numScores):
            probWgtList = scoreScoreLag1IsQualVal1Dec0DnWgtDisList[1:(i+1)][::-1] + [scoreScoreLag1IsQualVal1Dec0EqWgtDis] + scoreScoreLag1IsQualVal1Dec0UpWgtDisList[1:(self.numScores-i)]
            nrmProbWgtList = [probWgt * 1.0 / sum(probWgtList) for probWgt in probWgtList]
            self.transProbScoreCondScoreLag1IsQualVal1Dec0ListList.append(nrmProbWgtList)

        self.transProbScoreCondScoreLag1IsQualVal0Dec1ListList = []
        for i in range(self.numScores):
            probWgtList = scoreScoreLag1IsQualVal0Dec1DnWgtDisList[1:(i+1)][::-1] + [scoreScoreLag1IsQualVal0Dec1EqWgtDis] + scoreScoreLag1IsQualVal0Dec1UpWgtDisList[1:(self.numScores-i)]
            nrmProbWgtList = [probWgt * 1.0 / sum(probWgtList) for probWgt in probWgtList]
            self.transProbScoreCondScoreLag1IsQualVal0Dec1ListList.append(nrmProbWgtList)

        self.transProbScoreCondScoreLag1IsQualVal0Dec0ListList = []
        for i in range(self.numScores):
            probWgtList = scoreScoreLag1IsQualVal0Dec0DnWgtDisList[1:(i+1)][::-1] + [scoreScoreLag1IsQualVal0Dec0EqWgtDis] + scoreScoreLag1IsQualVal0Dec0UpWgtDisList[1:(self.numScores-i)]
            nrmProbWgtList = [probWgt * 1.0 / sum(probWgtList) for probWgt in probWgtList]
            self.transProbScoreCondScoreLag1IsQualVal0Dec0ListList.append(nrmProbWgtList)
    ...
    def epiInit(self):
        self.scoreIndex = np.argmax(np.random.multinomial(1, self.pmfScoreList))
        
        isQualProb = self.probIsQualCondScoreList[1][self.scoreIndex]
        self.isQual = np.argmax(np.random.multinomial(1, [1 - isQualProb, isQualProb]))

    def create_scoreIndex(self, decision):#, isQual):
        if self.isQual == 1:
            if decision == 1:
                scoreIndex = np.argmax(np.random.multinomial(1, self.transProbScoreCondScoreLag1IsQualVal1Dec1ListList[self.scoreIndex]))
            else:# decision == 0: 
                scoreIndex = np.argmax(np.random.multinomial(1, self.transProbScoreCondScoreLag1IsQualVal1Dec0ListList[self.scoreIndex]))
        else:#self.isQual == 0:
            if decision == 1:
                scoreIndex = np.argmax(np.random.multinomial(1, self.transProbScoreCondScoreLag1IsQualVal0Dec1ListList[self.scoreIndex]))
            else:# decision == 0: 
                scoreIndex = np.argmax(np.random.multinomial(1, self.transProbScoreCondScoreLag1IsQualVal0Dec0ListList[self.scoreIndex]))
        return scoreIndex

    def create_isQual(self, scoreIndex):#, decision):
        isQualProb = self.probIsQualCondScoreList[1][scoreIndex]
        isQual = np.argmax(np.random.multinomial(1, [1 - isQualProb, isQualProb]))
        return isQual
    def create_rewardVal(self, decision):#, decision):#how the decision impacting the distribution
        if decision == 1:
            if self.isQual == 1:
                rewardVal = self.rewardCoefIsQual1Dec1 * self.scoreIndex #* self.score + self.rewardEps
            else:
...
class EnvTrainRealGenerative(object):
    def __init__(self):
        self.path = None
...
#            self.groupProbList = config["groupProbList"]

            self.numScores = config["numScores"]
            self.minScore = config["minScore"]


            self.scoreScoreLag1IsQualVal1Dec1DnWgtDisList = config["scoreScoreLag1IsQualVal1Dec1DnWgtDisList"]
            self.scoreScoreLag1IsQualVal1Dec1EqWgtDis = config["scoreScoreLag1IsQualVal1Dec1EqWgtDis"]
            self.scoreScoreLag1IsQualVal1Dec1UpWgtDisList = config["scoreScoreLag1IsQualVal1Dec1UpWgtDisList"]
            

            self.scoreScoreLag1IsQualVal1Dec0DnWgtDisList = config["scoreScoreLag1IsQualVal1Dec0DnWgtDisList"]
            self.scoreScoreLag1IsQualVal1Dec0EqWgtDis = config["scoreScoreLag1IsQualVal1Dec0EqWgtDis"]
            self.scoreScoreLag1IsQualVal1Dec0UpWgtDisList = config["scoreScoreLag1IsQualVal1Dec0UpWgtDisList"]

            self.scoreScoreLag1IsQualVal0Dec1DnWgtDisList = config["scoreScoreLag1IsQualVal0Dec1DnWgtDisList"]
            self.scoreScoreLag1IsQualVal0Dec1EqWgtDis = config["scoreScoreLag1IsQualVal0Dec1EqWgtDis"]
            self.scoreScoreLag1IsQualVal0Dec1UpWgtDisList = config["scoreScoreLag1IsQualVal0Dec1UpWgtDisList"]

            self.scoreScoreLag1IsQualVal0Dec0DnWgtDisList = config["scoreScoreLag1IsQualVal0Dec0DnWgtDisList"]
            self.scoreScoreLag1IsQualVal0Dec0EqWgtDis = config["scoreScoreLag1IsQualVal0Dec0EqWgtDis"]
            self.scoreScoreLag1IsQualVal0Dec0UpWgtDisList = config["scoreScoreLag1IsQualVal0Dec0UpWgtDisList"]
            ...
            for i in range(self.numGroups):
                group = GroupEnv()
                group.init(self.numScores, self.minScore, self.numNnTrainSteps, self.scoreScoreLag1IsQualVal1Dec1DnWgtDisList[i], self.scoreScoreLag1IsQualVal1Dec1EqWgtDis[i], self.scoreScoreLag1IsQualVal1Dec1UpWgtDisList[i], self.scoreScoreLag1IsQualVal1Dec0DnWgtDisList[i], self.scoreScoreLag1IsQualVal1Dec0EqWgtDis[i], self.scoreScoreLag1IsQualVal1Dec0UpWgtDisList[i], self.scoreScoreLag1IsQualVal0Dec1DnWgtDisList[i], self.scoreScoreLag1IsQualVal0Dec1EqWgtDis[i], self.scoreScoreLag1IsQualVal0Dec1UpWgtDisList[i], self.scoreScoreLag1IsQualVal0Dec0DnWgtDisList[i], self.scoreScoreLag1IsQualVal0Dec0EqWgtDis[i], self.scoreScoreLag1IsQualVal0Dec0UpWgtDisList[i], pmfScoreList[i], probIsQualCondScore3DList[i], self.rewardCoefIsQual1Dec1List[i], self.rewardCoefIsQual0Dec1List[i], self.rewardCoefDec0List[i], initProbTrueList[i], transProbTrueList[i], rewardTrueList[i])
    ...
    def create_explicitModel_dp(self, pmfScoreList, probIsQualCondScore3DList):
        ...
        initProbTrueList = []
        initScoreProbArr = np.array([0.2 for i in range(5)]).reshape(5,1)#np.array(pmfScoreList[0]).reshape(5,1)
        ...
        initScoreProbArr = np.array([0.2 for i in range(5)]).reshape(5,1)#np.array(pmfScoreList[1]).reshape(5,1)
        initIsQualProbArr = np.array([0.5, 0.5]).reshape(1,2)
        initProbPerGroupArr = np.matmul(initScoreProbArr, initIsQualProbArr)
        initProbPerGroupArr = initProbPerGroupArr * 1.0 / np.sum(initProbPerGroupArr)
        initProbTrueList.append(initProbPerGroupArr)
    ...
    def feedback(self, action, nnTrainStepIndex):
        ...
        scoreIndexList = []
        for groupIndex in range(self.numGroups):
            scoreIndex = self.groupList[groupIndex].create_scoreIndex(action.decisionList[groupIndex])
            isQual = self.groupList[groupIndex].create_isQual(scoreIndex)
