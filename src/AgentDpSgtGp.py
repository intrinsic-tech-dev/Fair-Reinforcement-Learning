...
class AgentDpSgtGp(AgentBase):#AgentMbLnrFwdGatFwdLnrBwdGatBwd
    def __init__(self):
        self.path = None
    ...
    def read_config(self, identifier, rootDir, exprimentHyperConfigDir, optConfigCurDir, optDataCurDir, iptConfigHistDir, iptDataHistDir, curLocalTimeInMicro, device, numTrainEpisode, numInterPolicyUpdateEpisodes, numInterPolicyUpdateEvalEpisodes, numInterPolicyEvalEpisodes, numNnTrainSteps, isTrainPhase, evalType, envType, groupProbList, trialTrainIndex, useTrueModel, updateEpisodeIndexList, globalAgentEndEstEpisodeIndexList, estConstrForUpdate):#nnBatchSize, nnInputDim, nnHiddenDim, nnTargetDim, nnLayerDim):#, nStates, nActions):
        ...
        with open(iptConfigFileName) as json_file:
            ...
            self.timeLimitDpSgt = config["timeLimitDpSgt"]
    ...
    def compute_occuMeas_update(self, trainEpisodeIndex, policyUpdateIndex):#, trainEpisodeIndex, nnTrainStepIndex):
        ...
        if self.algoType == "DpSgtGp":
            self.solve_dpSgtGp_opt_update(trainEpisodeIndex, policyUpdateIndex)


    def solve_dpSgtGp_opt_eval(self, trainEpisodeIndex, policyEvalIndex):#, estDpCosntr):#estGroupProb, estDpConstr):#estTransProbFirstGroup, estTransProbSecondGroup, estRewardFirstGroup, estRewardSecondGroup, estDpConstr):
        firstGroupIndex = 0
        secondGroupIndex = 1
    ...
    def solve_dpSgtGp_opt_update(self, trainEpisodeIndex, policyUpdateIndex):#, estDpCosntr):#estGroupProb, estDpConstr):#estTransProbFirstGroup, estTransProbSecondGroup, estRewardFirstGroup, estRewardSecondGroup, estDpConstr):
        firstGroupIndex = 0
        secondGroupIndex = 1
        if True:
            ...
            qcqp.setParam(GRB.param.NonConvex, 2)
            qcqp.setParam(GRB.param.MIPGap, self.mipGap)
            qcqp.setParam(GRB.param.TimeLimit, self.timeLimitDpSgt)
            qcqp.setParam(GRB.param.DualReductions, 0)
