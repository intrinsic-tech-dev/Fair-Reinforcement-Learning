...
from MemMgmt import MemMgmtOptRLFairTrain, MemMgmtOptRLFairEval
import torch
from VisRLFairTrain import VisRLFairTrain
...
class PipelineRLFairVis(Pipeline):
    ...
    def create_nodes(self, identifier, projDir, rootDir):
        with open(rootDir + "config/" + __class__.__name__ + identifier  + ".json") as json_file:
            ...
            self.pipeEvalLocalTimeInMicro = sys.argv[2] if len(sys.argv) > 0 else None
            self.pipeTrainLocalTimeInMicroList = sys.argv[3:] if len(sys.argv) > 0 else None
            self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

            self.projDir = projDir
            self.rootDir = rootDir
            self.identifier = identifier
            self.visTrain = VisRLFairTrain()
            ...
            self.iptTrainDataHistDirList = [projDir + 'hist/' + self.pipeTrainClassNameId + "/" + pipeTrainLocalTimeInMicro + "/data/ActorCriticsBackpropTrain/"  if pipeTrainLocalTimeInMicro != None else None for pipeTrainLocalTimeInMicro in self.pipeTrainLocalTimeInMicroList]
